...
module Database
  ...
  class Model
    ...
    def self.length
      Database::Model.execute("SELECT count(*) FROM #{self.class.to_s + 's'}").map do |row|
          self.new(row)
      end
    end
    def self.where(query, *args)
      Database::Model.execute("SELECT * FROM #{self.class.to_s + 's'} WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
    ...
    def self.all
    name = (self.to_s)+'s'
    Database::Model.execute("SELECT * FROM #{name}").map do |row|
      self.new(row)
      end
    end
